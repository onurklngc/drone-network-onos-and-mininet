...
def update_station_locations_on_mn(vehicle_data_list):
    for sumo_vehicle in vehicle_data_list:
        ...
        #     distance= mn_sta.get_distance_to(mn_sta.wintfs[0].associatedTo.node)
        #     if distance>
...
def add_bs(net, bs_location, bs_id=s.BS_ID_OFFSET):
    ...
    if s.TASK_ASSIGNER_SERVER == "BS_HOST":
        Simulation.set_task_assigner(Simulation.bs_host)
    net.bs_map[bs_id] = bs
    return bs, Simulation.bs_host


def add_bs_links(net, bs, bs_host, ap, controller_host, link_manager):
    global medium_id
    ...
    net.addLink(controller_host, ap, bw=100, delay='0ms')
...
def create_topology(drone_mover):
    ...
    for drone_id in range(s.NUMBER_OF_DRONES):
        ...
        if drone_id == Simulation.drone_id_close_to_bs:
            Simulation.controller_host = new_net.addHost('ch%d' % s.CONTROLLER_HOST_ID,
                                                         mac='00:10:00:00:00:' + hex(s.CONTROLLER_HOST_ID).split(
                                                             'x')[-1].zfill(2))
            if s.TASK_ASSIGNER_SERVER == "CONTROLLER_HOST":
    ...
    add_bs_links(new_net, bs, bs_host, drone_aps[Simulation.drone_id_close_to_bs], Simulation.controller_host,
                 link_manager)
    ...
    new_net.build()
    Simulation.nat_host = new_net.addNAT(linkTo=bs.name)
    Simulation.nat_host.configDefault()
    ...
    for vehicle in Simulation.connecting_vehicles.copy().values():
        ...
        if sta.wintfs[0].associatedTo:
            ...
            if "Not connected" in link_connection_output:
                logging.error(f"{sta} is not connected to {ap_intf} contrary to config. "
                ...
                Simulation.add_to_connected_vehicles(vehicle)
        else:
            logging.error(f"{sta} lost its network access even before connecting.")
            Simulation.set_vehicle_as_disconnected(vehicle.sumo_id)
