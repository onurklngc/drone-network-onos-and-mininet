...
from settings import FILE_REQUEST_LISTEN_PORT, ITG_LOG_SERVER_IP, MININET_IP, HOST_AVAILABILITY_LISTEN_PORT, \
    ITG_SENDER_LOG_PATH, USE_IPERF
...
def inform_host_is_available(sender_ip, tmp_host_ip, flow_id):
...
def send_file(requesting_host_ip, requested_file_id, requested_file_size, flow_id):
    if USE_IPERF:
    ...
    inform_host_is_available(host_ip, requesting_host_ip, flow_id)
...
if __name__ == '__main__':
    ...
    host_ip = sys.argv[1]
    if host_ip.split(".")[-2] != "0":
        # It is switch's dummy host representing cache component
        host_id = "s" + host_ip.split(".")[-2]
        debug("host_ip " + host_ip)
        debug("Host is awake")
        listen_file_requests()
    else:
        # Regular host
