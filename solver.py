...
record_file = "records/vehicle_speed_v6/40/lambda8_seed57"

TIME_WINDOW = 300
MAX_COMBINATION_TO_TRY_PER_WINDOW = 200000
processor_records = {}
generator_records = {}
ap_records = {}
processors = {}
...
def get_simulation_record(filename):
    ...
    for sumo_id, vehicle in sim_record.vehicles.items():
        ...
        if not vehicle.no_new_task_start_time:
            vehicle.no_new_task_start_time = min(sim_record.sim_end_time, vehicle.departure_time)
...
def get_estimated_tx_time_station_to_cloud(task_size, given_time, owner_object, temp_earliest_next_task_start_tx_time,
    ...
    general_load = temp_traffics[given_time].sum() / 2 / NUMBER_OF_STATIONS
    logging.debug(f"General load effect is {general_load:.1f}")
    ap_load = 1 + general_load
    ...
    if sta_side_tx_time is None:
        ...
        return
    load_on_cloud_on_time_t = temp_traffics[given_time][CLOUD] + 1 #+ general_load
    cloud_side_tx_time = task_size * load_on_cloud_on_time_t / 2500
    tx_time = ceil(max(sta_side_tx_time, cloud_side_tx_time))
    if tx_time + given_time > record.sim_end_time:
        return
    # connection delay related to average number of handovers
    tx_time += 2
    if "/20/" in record_file:
        tx_time += 2 * 3 * (100 / 342)
    elif "/40/" in record_file:
        tx_time += 2 * 5.1 * (120 / 363)
    tx_time = ceil(tx_time)
    # logging.info(f"cloud tx time: {tx_time} ")
    extra_delay = add_delay_to_previous_tasks(given_time, tx_time, ap_moment.associated_stations, [], ap_load,
                                              load_on_cloud_on_time_t, temp_earliest_next_task_start_tx_time,
                                              is_cloud=True)
    tx_time += extra_delay
    if tx_time + given_time > record.sim_end_time:
        return
...
def get_estimated_tx_time_station_to_station(task_size, task_start_time, owner_object, processor_object, task_pair_bw,
    ...
    general_load = temp_traffics[tx_start_time].sum() / NUMBER_OF_STATIONS
    logging.debug(f"General load effect is {general_load:.1f}")
    generator_ap_load = 1 + general_load
    processor_ap_load = 1 + general_load
    ...
    # connection delay related to average number of handovers (multiplied by 2 because both vehicles are moving)
    tx_time += 3
    if "/20/" in record_file:
        tx_time += 2 * 3 * 3 * (100 / 342)
    elif "/40/" in record_file:
        tx_time += 2 * 3 * 5.1 * (150 / 363)
    tx_time = ceil(tx_time)
    extra_delay = add_delay_to_previous_tasks(tx_start_time, tx_time, generator_ap_moment.associated_stations,
                                              ...
                                              processor_ap_load, temp_earliest_next_task_start_tx_time, is_cloud=True)
    tx_time += extra_delay
...
def add_delay_to_task(task_no, next_task_tx_start_time, load_level):
    ...
    # logging.info(f"Conflict load level: {load_level}")
    if task_details[1] > 300:
        return 0
    if next_task_tx_start_time >= tx_end_time:
        return 0
    conflict_duration = (tx_end_time - next_task_tx_start_time) * 3 / (load_level+1)
    if "/20/" in record_file:
        conflict_duration += 2 * 3 * 3 * (conflict_duration / 342)
    elif "/40/" in record_file:
        conflict_duration += 2 * 3 * 5.1 * (conflict_duration / 363)
    extra_delay = conflict_duration / max(load_level - 1, 1)
...
def add_delay_to_previous_tasks(given_time, tx_time, src_stations, dest_stations, src_ap_load, dest_ap_load,
                                temp_earliest_next_task_start_tx_time, is_cloud=False):
    max_extra_delay = 0
    extra_delay = 0
    for task_no, decision in enumerate(solution):
        ...
        if is_cloud and decision == Action.CLOUD:
            ...
            extra_delay = add_delay_to_task(task_no, given_time, dest_ap_load)
            temp_earliest_next_task_start_tx_time[decision.sumo_id] += extra_delay
    ...
    for decision_no in range(decision_index):
        ...
        if is_cloud and previous_decision == Action.CLOUD:
            ...
            extra_delay = add_delay_to_task(next_task_index + decision_no, given_time, dest_ap_load)
            temp_earliest_next_task_start_tx_time[previous_decision.sumo_id] += extra_delay
        max_extra_delay = max(extra_delay, max_extra_delay)
...
def evaluate_combination(best_total_prioritized_penalty, tasks_to_be_decided, combination):
    ...
    for decision_index, task in enumerate(tasks_to_be_decided):
        ...
        if decision == Action.CLOUD:
            ...
            estimated_end_time = tx_start_time + estimated_tx_time + process_time
...
def parse_cli_arguments():
    ...
    parser.add_argument("-l", "--log-level", dest="log_level",
                        ...
                        help="Set the logging level", default=LOG_LEVEL)
    ...
    while next_task_index < number_of_tasks:
        ...
        while possibilities > MAX_COMBINATION_TO_TRY_PER_WINDOW:
            ...
            if possibilities > 1:
                min_possibilities = possibilities
        if is_shrunk_window:
            logging.debug(f"Readjusting window: {window_start_time}-{window_end_time} "
                          f"(size:{window_end_time - window_start_time})")
            logging.debug(f"Number of possible pairings: {num_of_decisions}, possibilities: {possibilities}")
            if window_start_time == window_end_time:
                MAX_COMBINATION_TO_TRY_PER_WINDOW = min_possibilities + 1
                logging.warning(f"Increasing MAX_COMBINATION_TO_TRY_PER_WINDOW since it is not sufficient: "
                                f"New value:{MAX_COMBINATION_TO_TRY_PER_WINDOW}")
