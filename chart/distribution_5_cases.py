...
def sub_plot(ax, penalty_data_list, method_name, plot_index, case_ordering):
    ...
    for i in range(3):
        ax.text(sub_x[i]+0.01, total_delays[i], tags[i], ha="center", va="bottom", size=12)
...
def plot(penalty_data_list, case_ordering):
    ...
    plt.savefig(f"chart/plots/request_vs_delay_distribution.pdf", dpi=300, bbox_inches='tight')
    # plt.savefig(f"chart/plots/request_vs_delay_distribution.png", dpi=300, bbox_inches='tight')
    plt.show()
