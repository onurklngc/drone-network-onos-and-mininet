...
def plot_graph(tasks, title=""):
    ...
    ax1.set_xlabel('Task ID', fontsize=36)
    ax1.set_ylabel(r"$t_{total,i}$ (s)", fontsize=50)
    ax1b = ax1.twinx()
    color = 'tab:orange'
    ...
    ax2.set_ylabel(r'$t_{total,i}$ (s)', fontsize=36)
    ax2.set_xlabel('Frequency', fontsize=36)
    ax2.set_ylim(0, 300)
    # ax2.set_xlim(0, 0.1)
    ax2.grid()
    plt.title(title, fontdict=None, loc='left', pad=75,size=42)
...
if __name__ == '__main__':
    ...
    plot_graph(adaptive_tasks, r"ADP - Case 1.3: $1/\lambda = 15$")
    adaptive_tasks = get_specific_tasks(result_data, "Adaptive", "10")
    adaptive_tasks.sort(key=lambda x: x.no)
    plot_graph(adaptive_tasks, r"ADP - Case 1.2: $1/\lambda = 10$")
    adaptive_tasks = get_specific_tasks(result_data, "Adaptive", "5")
    adaptive_tasks.sort(key=lambda x: x.no)
    plot_graph(adaptive_tasks, r"ADP - Case 1.1: $1/\lambda = 5$")
