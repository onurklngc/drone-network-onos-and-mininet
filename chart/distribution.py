...
from chart.utils import get_multiple_files, get_specific_tasks, \
    get_average_system_times, PROCESS_SPEED_FILE_LIST, ORDERING, VEHICLE_SPEED_FILE_LIST, REQUEST_INTERVAL_FILE_LIST
...
def plot(penalty_data_list, case_ordering):
    fig, ax = plt.subplots()
    sub_plot(ax, penalty_data_list, "Aggressive-Wait", 0, case_ordering)
    sub_plot(ax, penalty_data_list, "Aggressive", 1, case_ordering)
    sub_plot(ax, penalty_data_list, "Adaptive", 2, case_ordering)
    sub_plot(ax, penalty_data_list, "Optimum", 3, case_ordering)

    plt.legend(
        [r'$\overline{t}_{tx}$', r'$\overline{t}_{process}$', r'$\overline{t}_{pool}$', r'$\overline{t}_{queue}$'])
    main_x = np.arange(4)
    plt.xticks(main_x, ['AGG-1', 'AGG-2', 'ADP', 'Q-OPT'])
    plt.show()
